...
bool Agent::wantsToRoute(Agent &agent)
{
  if (d_type == 0 || d_type == 2)
    return d_wantsToRoute;
  else if (d_type == 1)
    return (bool)d_memoryTable[agent.id()];
  else
  {
    if (d_memoryTable[agent.id()] == 1)
      return d_wantsToRoute;
    else
      return false;
  }
...
void Agent::updateRouteTable(vector<Agent> &population, World &world)
{
  flushRouteTable();

  for (Agent &agent : population)
  {
    if (isNeighbour(agent, world))
    {
      d_routeTable[agent.id()] = TableEntry{agent.id(), 1};
    }
    else
    {
      d_routeTable[agent.id()] = TableEntry{-1, 99999999};
    }
  }
  d_routeTable[this->id()] = TableEntry{this->id(), 0};
}

bool Agent::advertiseTable(vector<Agent> &population, World &world)
{
  bool improved = false;
  for (Agent &agent : population)
  {
    if (isNeighbour(agent, world))
    {
      if (agent.recieveTable(d_routeTable, this->id(), population))
      {
        improved = true;
        agent.setTableTo(true);
      }
    }
  }
  return improved;
...
bool Agent::recieveTable(vector<TableEntry> &table, int senderID, vector<Agent> &population)
{
  int idx = 0;
  bool improved = false;
  if (!population[senderID].wantsToRoute(*this))
  {
    if (d_type != 0)
    {
...
bool Agent::sendMessage(Agent &reciever, vector<Agent> &population)
{
  ++d_messagesTotal;
  ...
  {
    ++d_messagesSent;
    population[nextHop].routeMessage(reciever, population);
    return true;
  }
  else
  {
...
void Agent::routeMessage(Agent &reciever, vector<Agent> &population)
{
  // cout << this->id() << " sending to " << reciever.id() << "\n";
